...
def get_texts(text_category):
    if text_category == "spain" or text_category == 'portugal':
        with open('./data/text_config.json', 'r', encoding='utf-8') as f:
            texts = json.load(f)
            return texts['welcome'][text_category]
    else:
        with open('./data/text_config.json', 'r', encoding='utf-8') as f:
            texts = json.load(f)
            return texts['text_modules'][text_category]
...
def update_texts(text_category, text):
    if text_category == "spain" or text_category == 'portugal':
        with open('./data/text_config.json', 'r', encoding='utf-8') as f:
            texts = json.load(f)
            texts['welcome'].pop(text_category)
            texts['welcome'][text_category] = text
        with open('./data/text_config.json', 'w', encoding='utf-8') as f:
            json.dump(texts,  f,ensure_ascii=False, indent=4)
    else:
        with open('./data/text_config.json', 'r', encoding='utf-8') as f:
            texts = json.load(f)
            texts['text_modules'].pop(text_category)
            texts['text_modules'][text_category] = text
        with open('./data/text_config.json', 'w', encoding='utf-8') as f:
            json.dump(texts,  f,ensure_ascii=False, indent=4)
